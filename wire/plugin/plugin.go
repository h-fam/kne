...
func cmdAdd(args *skel.CmdArgs) error {
	...
	meshnetClient := wpb.NewLocalClient(conn)

	log.Infof("Retrieving local pod information from meshnet daemon")
	localPod, err := meshnetClient.Get(ctx, &wpb.PodQuery{
	...
	for _, link := range localPod.Links { // Iterate over each link of the local pod
		...
		peerPod, err := meshnetClient.Get(ctx, &wpb.PodQuery{
			Name:   link.PeerPod,
			KubeNs: string(cniArgs.K8S_POD_NAMESPACE),
		})
		if err != nil {
		...
		if isAlive { // This means we're coming up AFTER our peer so things are pretty easy
			...
			if peerPod.SrcIp == localPod.SrcIp { // This means we're on the same host
				...
				if iExist && pExist { // If both link exist, we don't need to do anything
					...
					isSkipped, err := meshnetClient.IsSkipped(ctx, &wpb.SkipQuery{
						Pod:    localPod.Name,
						Peer:   peerPod.Name,
						KubeNs: string(cniArgs.K8S_POD_NAMESPACE),
					})
				...
				remoteClient := wpb.NewRemoteClient(remote)
			...
			ok, err := meshnetClient.Skip(ctx, &wpb.SkipQuery{
				Pod:    localPod.Name,
				Peer:   peerPod.Name,
				KubeNs: string(cniArgs.K8S_POD_NAMESPACE),
			})
...
func cmdDel(args *skel.CmdArgs) error {
	...
	for _, link := range localPod.Links { // Iterate over each link of the local pod
		...
		ok, err := meshnetClient.SkipReverse(ctx, &wpb.SkipQuery{
			Pod:    localPod.Name,
			Peer:   link.PeerPod,
			KubeNs: string(cniArgs.K8S_POD_NAMESPACE),
		})
